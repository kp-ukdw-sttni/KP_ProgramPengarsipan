...
const submitReject = () => {
    ...
    working.value = true
    router.post(routeFn('peminjaman.reject', rejectTarget.value.id), {}, {
        preserveScroll: true,
        onFinish: () => {
            working.value = false
...
<template>
    <AuthenticatedLayout title="Persetujuan Akses">
        ...
        <div class="mt-5 mb-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            >
                <span class="text-xs font-medium text-white/85">{{ card.label }}</span>
        ...
        <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div v-if="items.length === 0" class="py-16 text-center">
                <p class="text-sm font-medium text-gray-500">Tidak ada pengajuan.</p>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-100 text-sm">
                    <thead>
                        <tr class="border-b border-gray-200 bg-gray-50 text-[11px] uppercase tracking-wider text-gray-500">
                            <th scope="col" class="px-5 py-3.5 text-left font-bold">Pemohon</th>
                            ...
                            <th scope="col" class="px-5 py-3.5 text-left font-bold">Waktu Pengajuan</th>
                    ...
                    <tbody class="divide-y divide-gray-100 text-gray-700">
                        <tr v-for="p in items" :key="p.id" class="transition-colors duration-150 hover:bg-indigo-50/40">
                            <td class="px-5 py-4">
                                ...
                                <div class="mt-0.5 text-[11px] text-gray-400">{{ p.user?.email }}</div>
                            </td>
                            <td class="max-w-xs px-5 py-4">
                                <div class="leading-tight font-semibold text-gray-800">{{ p.arsip?.judul }}</div>
                            ...
                            <td class="whitespace-nowrap px-5 py-4 text-right">
                                <div v-if="p.status_approval === 'Pending'" class="flex items-center justify-end gap-1.5">
                                    <button
                                        type="button"
                                ...
                                <span v-else-if="p.status_approval === 'Approved' && p.approver" class="text-[11px] text-gray-400">
        ...
        <Modal :show="!!approveTarget" max-width="sm" @close="approveTarget = null">
            <form v-if="approveTarget" @submit.prevent="submitApprove">
                <div class="border-b border-gray-100 px-6 py-4">
                    <h3 class="text-sm font-bold text-gray-900">Setujui Akses Dokumen</h3>
                    <p class="mt-0.5 text-xs text-gray-500">{{ approveTarget.arsip?.judul }}</p>
                </div>
                <div class="px-6 py-5">
                    <label class="mb-1 block text-[11px] font-semibold text-gray-500">
                        Durasi Akses (jam)
                    </label>
                    <Input v-model="approveForm.duration" type="number" min="1" max="168" />
                    <InputError :message="approveForm.errors.duration" />
                    <p class="mt-2 text-xs text-gray-400">
                        Maksimal 168 jam (7 hari). Pemohon dapat mengakses dokumen selama durasi ini.
                    </p>
                </div>
                <div class="flex items-center justify-end gap-2 border-t border-gray-100 px-6 py-4">
                    <SecondaryButton type="button" @click="approveTarget = null">Batal</SecondaryButton>
        ...
        <Modal :show="!!rejectTarget" max-width="sm" @close="rejectTarget = null">
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="mt-1 text-xs text-gray-500">
                            Pengajuan <span class="font-semibold">{{ rejectTarget.user?.name }}</span> untuk
                            <span class="font-semibold">{{ rejectTarget.arsip?.judul }}</span> akan ditolak.
                        </p>
                    </div>
                </div>
                <div class="mt-5 flex items-center justify-end gap-2">
                    <SecondaryButton type="button" @click="rejectTarget = null">Batal</SecondaryButton>
